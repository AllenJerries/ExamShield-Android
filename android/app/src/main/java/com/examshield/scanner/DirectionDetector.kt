    ...
    override fun onSensorChanged(event: SensorEvent) {
        when (event.sensor.type) {
            Sensor.TYPE_ROTATION_VECTOR -> {
                ...
                // Continuous, zero-lag per-frame azimuth update — the needle
                // rotates fluidly on EVERY rotation-vector sensor frame so the
                // compass readout tracks real turns instantly with no smoothing
                // and no hysteresis deadband.
